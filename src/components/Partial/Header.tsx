  ...
  return (
    <nav className='flex flex-wrap flex-row h-[8vh] w-[100vw] lg:w-[60vw]'>
      ...
      {navigation.map((item, idx) => (
        <div key={idx} className='transition ease-in-out duration-500 flex-1 flex flex-col justify-center align-center p-2 hover:bg-lime-400 cursor-pointer'>
          <div className='flex flex-col'><Icon icon={item.icon} className='w-[100%]'/></div>
          <div className='flex flex-col'><span className='w-[100%] flex justify-center hidden'>{item.Name}</span></div>
        </div>
      ))}
      </div>
    </nav>
