...
const Aside: React.FC = () => {
  ...
  return (
    <nav id="Aside" ref={reference} className='flex flex-wrap flex-col absolute bg-stone-300 top-[8vh] h-[100%] text-lime-950 w-[100vw] lg:w-[20vw] z-50 m-auto lg:left-0 sm:left-[-100vw] transition ease-in-out'>
      <div className='flex flex-wrap flex-col w-[100%]'>
      <div  className='flex flex-1 p-2 border-4 border-solid border-lime-700 flex-row align-center bg-lime-600'>
          <div className='flex flex-col justify-center p-2'><Icon icon="ep:menu" /></div>
          <div className='flex flex-col justify-center'>Menu</div>
      </div>
      {Menu.map((item, idx) => (
        <div key={idx} className='transition ease-in-out duration-500 flex-1 flex flex-row align-center p-2 border-b-4 border-b-solid border-b-lime-800 hover:bg-stone-400'>
